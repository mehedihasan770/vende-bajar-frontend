...
const sidebarLinks = [
  ...
  { name: 'Returns', href: '/dashboard/admin/returns', icon: FileText },
  ...
  return (
    <div className="p-4 md:p-5 xl:p-6">
      <div className="mb-5 xl:mb-6">
        <h1 className="text-xl md:text-2xl font-black text-gray-900 tracking-tight">Orders & Logistics</h1>
        <p className="text-[13px] md:text-sm text-gray-500 mt-1 font-medium">Manage customer orders, returns, and shipping configurations.</p>
      </div>
        ...
        <aside className="w-full xl:w-48 flex-shrink-0 xl:sticky xl:top-6 self-start">
          <nav className="flex flex-row xl:flex-col gap-1 overflow-x-auto pb-2 xl:pb-0 custom-scrollbar">
            {sidebarLinks.map((link) => {
              const isActive = pathname === link.href || (link.href !== '/dashboard/admin/orders' && pathname.startsWith(link.href));
