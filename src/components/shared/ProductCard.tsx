...
const ProductCard = ({ item, index = 0 }: ProductCardProps) => {
  ...
  const displayPrice = item.salePrice && item.salePrice > 0 ? item.salePrice : item.basePrice;
  ...
  const imageSrc = isValidUrl(item.thumbnail) ? item.thumbnail : 'https://placehold.co/600x400?text=No+Image';
          ...
          <div className="relative w-full h-full bg-[#F4F5F7] rounded-[19px] overflow-hidden isolate transform-gpu">
            ...
            <div className="absolute top-3 left-3 z-20 flex flex-col gap-1.5">
              <div className="bg-white/90 backdrop-blur-sm rounded-full px-2.5 h-5 flex items-center justify-center shadow-sm border border-gray-50">
                <span className="text-[8px] sm:text-[9px] font-bold tracking-wider text-accent uppercase leading-none mt-[1px]">{categoryName}</span>
              </div>
            </div>
